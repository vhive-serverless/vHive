...
performance bottleneck of colocating VMs on a single host. The motivation of this tool is to 
improve the parallelism of serverless architecture and commercial benefits for cloud vendors.
...
- A loader function issues requests to vHive step by step until the RPS reaches the maximum. At 
  ...
  function returns the metric before it.
...
- The tool can bind only 1 VM to a core for profiling and other VMs to a socket
...
A load step is divided into three phases: warm-up, profiling and cool-down. During the profile 
...
to profile user-defined counters followed by [the TopDown method](https://ieeexplore.ieee.org/document/6844459). 
...
-loadStep     FLOAT  The percentage of target RPS the benchmark loads at every step (default 5)
...
```
sudo env "PATH=$PATH" go test -v -timeout 99999s -run TestProfileIncrementConfiguration -args -funcNames cnn_serving -vmIncrStep 1 -maxVMNum 32 -l 1
```
