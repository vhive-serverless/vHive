...
echo "Deployment: waiting relay..."
kubectl wait --for=condition=Available --timeout=120s deployment/relay || { kubectl describe deployment/relay; exit 1; }
...
echo "Trigger: waiting default-relay-trigger..."
kubectl wait --for=condition=Ready --timeout=120s trigger/default-relay-trigger || { kubectl describe trigger/default-relay-trigger; exit 1; }
